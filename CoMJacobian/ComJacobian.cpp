...
{
	MatrixXd J;
	MatrixXd J_ref = MatrixXd::Zero(3,12);
	//total = 0.0;
	total = calcTotalMass(ulink,WAIST);
	vector<int> idy = kine->CustomLinkPath(zero, targetlink);
	const int ksize = idy.size();
	for(int to=0; to<ksize; to++){
		vector<int> idx = kine->CustomLinkPath(zero, idy[to]);

		const int jsize = idx.size();

		J.resize(3, jsize);

		J = calcCoMJacobian(zero, idx);

		J_ref = (ulink[to].mass*J) + J_ref;
...
MatrixXd CoMJacobian::calcCoMJacobian(int from, vector<int> idx)
{
	...
	//Matrix<double,3,1> target = ulink[idx.back()].p_com;
	Matrix<double,3,1> target = ulink[idx.back()].p_com - ulink[from].p;
	Matrix<double,3,12> J = MatrixXd::Zero(3,12);

	for(size_t i=0; i<jsize; i++)
	{
		int j = idx[i];
		Matrix<double,3,1> a = ulink[j].R * ulink[j].a;
		Matrix<double,3,1> b = a.cross(target - (ulink[j].p_com - ulink[from].p));
		//Matrix<double,3,1> b = a.cross(target - (ulink[j].p_com));
...
void CoMJacobian::fixedJacobian(vector<int> idx)
{
	...
	J_fix = calcfixedJacobian(zero, idx);
...
Matrix<double,3,12> CoMJacobian::resultCJ(int targetlink, Matrix<double,3,1> eff)
{
	Matrix<double,3,12> J;
	J = totalCoMJacobian(targetlink);
	vector<int> idx = kine->CustomLinkPath(zero, targetlink);
	fixedJacobian(idx);

	p_g = calcCoMPosition(zero);
	//p_g = calcCoM(ulink);
	//cout << p_g(1)*10 *-100;
	p_f = calcfixedlinkPosition(zero,targetlink);
	R_0 <<	1,0,0,
			0,1,0,
			0,0,1;
...
void CoMJacobian::calcInverse(int targetlink, Link target, Matrix<double,3,1> eff)
{
	...
	idx = kine->CustomLinkPath(zero,targetlink);
	Matrix<double,12,3> ref_J = Matrix<double,12,3>::Zero();
	Matrix<double,12,1> dp = Matrix<double,12,1>::Zero();
